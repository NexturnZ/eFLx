...
from utils.flexibility_prediction.baseline_policy import baseline_opt


"""
Function: optimize with regard to Reactive to e-taxi energy (R2E) manner
input parameters:
    t_start: request start time \hat{t}
    T_b: request duration
    tau: ramping duration
    T_c: control horizon
    date: data for which date is used
NOTE: cosntant["L"]*parameter["slot_length"]=30
"""
def R2I_policy(parameter, constant, J_trans:np.ndarray, J_idle:np.ndarray, P:np.ndarray, initial_state=None, date='2015-01-01', log=False):
    ...
    baseline_metric = {"J_trans":J_trans,
                       "J_idle":J_idle,
                       "P":P
    }
    ...
    if constant is None:
        constant = {
            ...
            "L2":6,
            "L_threshold":4,
            "num_of_taxis":4000, # 13000
            "num_of_regions":38
        }

    #region: randomly create initial fleet state
    if initial_state is None:
        ...
        for i in range(constant["num_of_taxis"]):
                ...
                if taxis.loc[i,"status"]==3:
                    ...
                    if arriving_time < max_occupied_length:
...
def R2I_opt(parameter, constant, initial_state, baseline_metric:dict, demand_data_path, log=False):
    ...
    #region: create intermediate variable
    ...
    for i in range(constant["num_of_regions"]):
        for l in range(constant["L"]):
            ...
            for t in range(T_c):
                ...
                # xp and xd decisions will NOT consume energy
                if l < constant["L2"]:
...
def R2I_policy_mpc(parameter, J_trans_baseline, J_idle_baseline, P_baseline,constant=None, initial_state=None, date='2015-01-01'): 
    ...
    for t in range(horizon):
        ...
        # apply optimization
        decision_new, metric_new, state_new = R2I_opt(
            ...
            if taxis.loc[n,"status"] == 3: # taxi is not occupied
                ...
                if len(current_trips[i])>0 and num_served[i]>=1: # have trip in region i
                    ...
                    if trip[1]/10-time > parameter["slot_length"]: # the trip doesn't finish in current time slot
                        ...
                        taxis.loc[n,"remaining_energy"] = max(0,taxis.loc[n,"remaining_energy"]) # type: ignore
                        taxis.loc[n,"update_status"] = 1
                elif charging_slot_free[i] > 0: # R2I, idle taxis go charging
                    taxis.loc[n,"remaining_energy"] += L_increase # type: ignore
                    taxis.loc[n,"status"] = 0
                    taxis.loc[n,"update_status"] = 1
                    charging_slot_free[i] -= 1

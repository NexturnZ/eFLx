...
""" Function: optimize the response policy
    Input:
        1. parameter:
...
"""
def potential_policy(parameter:dict, constant:dict, J_trans:np.ndarray, J_idle:np.ndarray, P:np.ndarray, initial_state:dict, date:str='2015-01-01',log:bool=False):
    ...
    decision, metric, state = potential_opt(parameter,constant,initial_state,baseline_metric,data_path,log=log)
    return decision, metric, state

def potential_opt(parameter:dict, constant:dict, initial_state:dict, baseline_metric:dict, demand_data_path:str, log:bool=False):
    #region: load data
    ...
    for slot in range(T_c):
        ...
        for slot_sub in range(slot_length):
            tmp = []
            with open(os.path.join(demand_data_path,str(parameter["t_start"]+parameter["t_hat"]+int(slot*slot_length)+slot_sub)+'_demand'),'r') as f:
                for line in f:
                    line = line.strip('\n')
                    line = line.split(',')
                    one=[]
    ...
    if "demand_prediction_error" in parameter.keys():
        if parameter["demand_prediction_error"] > 0:
            noise_pos = np.random.rand(estimatedpassengerdemand.shape[0], estimatedpassengerdemand.shape[1])
            noise_pos -= np.mean(noise_pos,0)
            demand_noise_pos = np.mean(estimatedpassengerdemand,0)*(1+noise_pos)*parameter["demand_prediction_error"]

            noise_neg = np.random.rand(estimatedpassengerdemand.shape[0], estimatedpassengerdemand.shape[1])
            noise_neg -= np.mean(noise_neg,0)
            demand_noise_neg = -np.mean(estimatedpassengerdemand,0)*(1+noise_neg)*parameter["demand_prediction_error"]

            estimatedpassengerdemand += demand_noise_pos + demand_noise_neg
            estimatedpassengerdemand[estimatedpassengerdemand < 0] = 0
            estimatedpassengerdemand = np.ceil(estimatedpassengerdemand)
    ...
    if T_b > 0:
        ################################################################
        # # OPTION 1: maximazing power reduction
        # m.setObjective(Delta_P,GRB.MAXIMIZE)

        # OPTION 2: maximizing energy reduction
        Delta_E = LinExpr()
        for i in range(constant["num_of_regions"]):
            for l in range(constant["L"]):
                for t in range(min(tau,T_c), min(tau+T_b,T_c)):
                    ...
                    Delta_E.add(-u[i,l,t])
        m.setObjective(Delta_E,GRB.MAXIMIZE)
        ###############################################################

    else:
        m.setObjective(J_trans,GRB.MAXIMIZE)
    #endregion

    # m.setParam(GRB.Param.TimeLimit, 3600)

    if log:
        m.setParam("LogFile","potential_log.log")
...
def potential_policy_mpc(parameter, J_trans_baseline, J_idle_baseline, P_baseline,constant=None, initial_state=None, date='2015-01-01'): 
...
if __name__ == '__main__':
    ...
    parameter = {
        ...
        "horizon":60, # length of MPC simulation (unit: min)
        "epsilon":0.0,
        "ita":np.inf,

...
"""
Function: optimize the reference policy
    Input:
        1. parameter:
        ...
        3. J_trans_ref (optional): total number of passenger served of baseline policy
        6. initial_state: initial {vacant, waiting, charging, occupied}
        7. date: data for which date is used
    Output:
        1. decision: optimized decision variables
        2. metric: metrics in interest of optimized decision
        3. state: optimized fleet state
        4. initial_state: the fleet state at the start of simulation
NOTE: cosntant["L"]*parameter["slot_length"]=30
"""
def baseline_policy(parameter, constant, J_trans_ref=None, initial_state=None, date='2015-01-01',log=False):
    ...
    if initial_state is None:
        if "hyperParameter" in parameter.keys():
            ...
            low_initial_battery = 15 # lower bound of initial battery level
        ...
        """
        taxis = pd.DataFrame(columns=["location","status","destination","arriving_time","remaining_energy","charging_status","update_status"],index=np.arange(constant['num_of_taxis']))
        taxis['destination'] = -1
        ...
        for slot in range(parameter['t_start']-1,parameter['t_start']+24,parameter["slot_length"]):
            perslot=[]
            for slot_sub in range(parameter["slot_length"]):
                with open(os.path.join(data_path,str(slot+slot_sub)+'_demand')) as f:
                    for line in f:
                        line = line.strip('\n')
                        line = line.split(',')
                        one=[]
        ...
        data=[]
        for t in range(parameter['t_start']-3,parameter['t_start']-1):
            if t >= 35:
                ...
                with open(file) as f:
                    for line in f:
                        line = line.strip('\n')
                        line = line.split(',')
                        if int(line[1]) > parameter['t_start']*10:
                            c = [int(float(line[0])),int(float(line[1])),int(float(line[2])),int(float(line[3]))]
                            data.append(c)
            else:
                break
        
        file = os.path.join(data_path, str(parameter['t_start']-1)+'_trips')
        with open(file) as f:
            for line in f:
                line = line.strip('\n')
                line = line.split(',')
        ...
        for i in range(len(data)):
            line = data[i]
            if line[1]<parameter['t_start']*10: # if the taxi finishes its current service during the current slot, it will still be unoccupied next time slot
                taxis.loc[i,"location"] = line[3]
                taxis.loc[i,"status"] = 3
                taxis.loc[i,"destination"] = -1
                taxis.loc[i,"arriving_time"] = -1
            elif line[1]>=parameter['t_start']*10 and line[1] < (parameter['t_start']+parameter["slot_length"])*10:
                taxis.loc[i,"location"] = line[3]
                taxis.loc[i,"status"] = 3
        ...
        for i in range(constant["num_of_taxis"]):
                energylevel = min(int(taxis.loc[i,"remaining_energy"]),constant["L"]-1) # type: ignore
                # energylevel = min(int(remaining_energy_taxi[i]/(2.5/ratio)),29)
                if taxis.loc[i,"status"]==3:
                    initial_state["vacant"][taxis.loc[i,"location"],energylevel] +=1 # type: ignore
                elif taxis.loc[i,"status"]==1:
                    initial_state["waiting"][taxis.loc[i,"location"],energylevel] +=1 # type: ignore
                elif taxis.loc[i,"status"] == 0:
                    # print (location_of_taxi[i],int(remaining_energy_taxi[i] / 2.5))
                    initial_state["charging"][taxis.loc[i,"location"],energylevel] += 1 # type: ignore
                elif taxis.loc[i,"status"] == 2:
                    arriving_time = int(np.floor((taxis.loc[i,"arriving_time"]/10-parameter["t_start"])/parameter["slot_length"])) # type: ignore
                    if arriving_time == 0:
                        initial_state["vacant"][taxis.loc[i,"location"],energylevel] +=1 # type: ignore
                    elif arriving_time < max_occupied_length:
                        initial_state["occupied"][taxis.loc[i,"location"],energylevel,arriving_time] += 1 # type: ignore
        
        # initial_state['vacant'] = np.array(initial_state['vacant'])
        # initial_state['charging'] = np.array(initial_state['charging'])
        # initial_state['waiting'] = np.array(initial_state['waiting'])
        # initial_state["occupied"] = np.zeros((constant["num_of_regions"],constant["L"],1)) # current occupied taxis which will be vacant in region i at time t with battery l

    decision, metric, state = baseline_opt(
        parameter=parameter,
        constant=constant,
        initial_state=initial_state,
        demand_data_path=data_path,
        J_trans_ref=J_trans_ref,
        log=log)
    
    return decision, metric, state, initial_state

def baseline_opt(parameter, constant, initial_state, demand_data_path, J_trans_ref=None, log=False):
    if "ref_charging_penalty" not in parameter.keys():
        ref_charging_penalty = "none"
    else:
        ref_charging_penalty = parameter["ref_charging_penalty"]
    
    #region: data loading
    data_path = os.path.join(ROOT, 'data')
    
    slot_length = parameter["slot_length"]
    T_c = int(np.ceil(parameter["T_c"]/slot_length))
    ...
    for slot in range(T_c):
        with open(os.path.join(data_path,'transition', str(slot+parameter["t_start"]))) as f:
            data=[]
            for line in f:
                line = line.strip('\n')
                line= line.split(',')
    ...
    # travelingTime = []
    # fopen = open(os.path.join(data_path, 'travelingtime_region'),'r')
    # for k in fopen:
    #     k=k.strip('\n')
    #     k=k.split(',')
    #     one =[]
    #     for value in k:
    #         one.append((float(value)))
    #     travelingTime.append(one)
    # fopen.close()
    # travelingTime = np.array(travelingTime,dtype="int")
    ...
    D={} # number of taxis with battery l at charging stations in region i.
        ...
        for l in range(constant["L"]):
            ...
            for t in range(T_c):
                ...
                if l==0:
                    ...
                    for j in range(constant['num_of_regions']):
                        if j in reachlist[i]: # the destination region is not reachable in one time slot,
                            if l >= 1:
                                V[i,l-1,t+1].add(transition[t][j][i][l]*S[j,l,t]) # POET equation (3)
                            else:
                                V[i,l,t+1].add(transition[t][j][i][l]*S[j,l,t]) # POET equation (3)
                        else: # the destination region is not reachable in one time slot
                            k = travelingTime[j,i]
    ...
    # taxis with full battery does not charge
    for i in range(constant["num_of_regions"]):
        for j in reachlist[i]:
    ...
    obj_charging = LinExpr() # minor objective for fleet sustainability
    for i in range(constant["num_of_regions"]):
        for l in range(constant["L"]):
            for t in range(min(tau,T_c), min(tau+T_b,T_c)):
                # Delta_E.add(baseline_metric["P"][i,l,t]-u[i,l,t])
    ...
    #     sum(
    #         sum(
    #             u[i,l,t] for i in range(constant["num_of_regions"])
    #         ) for l in range(constant["L"])
    #     ) for t in range(T_c)
    # )
    # obj = obj_trans+alpha*obj_charge

    if ref_charging_penalty=="negative":
        beta = 1e-3
        m.setObjective(obj_trans + beta*obj_charging, GRB.MAXIMIZE) 
    elif ref_charging_penalty=="positive":
        beta = 1e-3
        m.setObjective(obj_trans + beta*obj_charging, GRB.MAXIMIZE)
    else:
        m.setObjective(obj_trans, GRB.MAXIMIZE)
    # m.setObjective(obj,GRB.MAXIMIZE)

    # m.setParam(GRB.Param.TimeLimit, 3600)

    if log:
        m.setParam("LogFile","baseline_log.log")
    ...
    xp_val = np.zeros((constant["num_of_regions"],constant["num_of_regions"],constant["L"],T_c))
    xd_val = np.zeros((constant["num_of_regions"],constant["num_of_regions"],constant["L"],T_c))
    yp_val = np.zeros((constant["num_of_regions"],constant["num_of_regions"],constant["L"],T_c))
    yd_val = np.zeros((constant["num_of_regions"],constant["num_of_regions"],constant["L"],T_c))
    served_val = np.zeros((constant["num_of_regions"],T_c))
    u_val = np.zeros((constant["num_of_regions"],constant["L"],T_c))
    ...
    vacant = np.zeros((constant["num_of_regions"],constant["L"],T_c+1))
    charging = np.zeros((constant["num_of_regions"],constant["L"],T_c+1))
    waiting = np.zeros((constant["num_of_regions"],constant["L"],T_c+1))
    supply = np.zeros((constant["num_of_regions"],constant["L"],T_c+1))
    for i in range(constant['num_of_regions']):
        for l in range(constant['L']):
            ...
            for t in range(1,T_c+1):
                ...
                waiting[i,l,t] = sum( (x[j,i,l,t-1].x + xd[j,i,l,t-1].x ) for j in reachlist[i] )-u[i,l,t-1].x # take round after calculation
...
"""
    occupied = np.zeros((constant["num_of_regions"],constant["L"],T_c+1))
    for i in range(constant['num_of_regions']):
        for l in range(constant['L']):
            for t in range(T_c+1):
    ...
    metric = {"num_charging":u_val,"J_trans":served_val}
    ...
    for t in range(T_c):
        for i in range(constant['num_of_regions']):
            total_demand += estimatedpassengerdemand[i][t]
...
def baseline_policy_mpc(parameter, constant=None, initial_state=None, date='2015-01-01'):
    ...
    if initial_state is None:
        ...
        for i in range(constant["num_of_regions"]):
            ...
            for j in range(constant["num_of_regions"]):
                one.append(0.0)
            initial_demand.append(one)
        for slot in range(parameter['t_start']-1,parameter['t_start']+24):
            perslot=[]
    ...
    for t in range(horizon):
        time = t*parameter["slot_length"]+parameter["t_start"]
        print('********************************* ', time, " ******************************************")
        ...
        c_parameter["t_start"] = time

        # apply optimization
        decision_new, metric_new, state_new = baseline_opt(
        ...
        for n in range(constant['num_of_taxis']):
            l = int(min(taxis.loc[n,"remaining_energy"]/(idle_dis_dt/ratio),constant['L']-1))
            i = taxis.loc[n,"location"] # type: ignore
        ...
        for n in range(constant['num_of_taxis']):
            if taxis.loc[n,"status"] == 3: # taxi is not occupied
                ...
                if len(current_trips[i])>0 and num_served[i]>=1: # have trip in region i
                    ...
                    taxis.loc[n,"remaining_energy"] -= idle_dis_dt / ratio
                    taxis.loc[n,"location"] = j
                    taxis.loc[n,"update_status"] = 1
...
if __name__ == '__main__':
    ...
    parameter = {
        "t_start":960, # time of simulation starts (unit: min) 1020
        "t_hat":20, # time of beginning of balancing requests (unit: min)
        "tau":40, # ramping down duration (unit: min)
        "T_b":120, # service duration (unit: min)
        "T_c":180, # length of control horizon of upper problem (unit: min)
        "slot_length":20, # length of timeslot (should be multiple of 10, unit: min)
        "horizon":3000, # length of MPC simulation (unit: min)
        "epsilon":0.0,
        "ita":np.inf,
    ...
    decision_baseline, metric_baseline, state_baseline, taxi_state = baseline_policy_mpc(
        parameter=parameter,
        constant=constant,
        date=date
    )
    ...
    num_of_regions = 38
    L = 30
    p_baseline = np.zeros(int(np.ceil(parameter["T_c"]/10)))
    for t in range(int(np.ceil(parameter["T_c"]/10))):
        for i in range(num_of_regions):
            for l in range(L):
                p_baseline[t] += metric_baseline['num_charging'][i,l,t]
    print("average power of baseline policy is: ", p_baseline)

    J_trans = 0
    for t in range(int(np.ceil(parameter["T_c"]/10))):
        for i in range(num_of_regions):
            J_trans += metric_baseline['J_trans'][i,t]
    print("Number of passenger served by baseline policy is: ", J_trans)
    ...
    # result_path = ROOT.joinpath("results")
    # file_name = "test"
    # with open(str(result_path.joinpath(file_name)),'wb') as f:
